...
export async function GET(
	...
	try {
		...
		if (role !== 'admin' && role !== 'developer' && meme.user_id !== userId) {
...
export async function PATCH(
	...
	try {
		...
		if (validated.action === 'schedule') {
			...
			result = await scheduleMeme(
				id,
				validated.scheduledFor!.getTime(),
				scheduledPost.id,
			);
		} else {
		...
		if (_error instanceof Error && _error.name === 'ZodError') {
			const zodError = _error as unknown as {
...
export async function DELETE(
	...
	try {
		...
		// Owners can only delete if still pending. Admins/Developers can delete anything.
