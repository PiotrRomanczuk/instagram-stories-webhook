...
export async function GET(request: NextRequest) {
    try {
        ...
        console.log(`[API] 🚀 Process request received${postId ? ` for post ${postId}` : ''}`);
        ...
        if (!isCron && !isUserAuth) {
            console.error('🔒 Unauthorized attempt: Invalid or missing secret/session');
        ...
        if (postId && !isCron && isUserAuth) {
            ...
            if (error || !post || post.user_id !== session.user.id) {
                console.error(`🔒 Unauthorized post-immediately attempt by ${session.user.id} for post ${postId}`);
        ...
        if (postId && result.processed === 0) {
            console.warn(`[API] ⚠️ Post ${postId} was not processed: ${result.message}`);
            return NextResponse.json({ 
                error: result.message || 'Post could not be processed (it might be locked or already finished)',
                ...result 
            }, { status: 400 });
        }
