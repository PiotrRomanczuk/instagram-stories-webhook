...
export default function AdminMemesPage() {
	...
	const fetchMemes = useCallback(async () => {
		...
		try {
			const url =
				filter === 'all' ? '/api/memes' : `/api/memes?status=${filter}`;
			...
			if (filter === 'all') {
				const newStats = {
					...
					pending: data.memes.filter(
						(m: MemeSubmission) => m.status === 'pending',
					).length,
					approved: data.memes.filter(
						(m: MemeSubmission) => m.status === 'approved',
					).length,
					scheduled: data.memes.filter(
						(m: MemeSubmission) => m.status === 'scheduled',
					).length,
					published: data.memes.filter(
						(m: MemeSubmission) => m.status === 'published',
					).length,
					rejected: data.memes.filter(
						(m: MemeSubmission) => m.status === 'rejected',
					).length,
				};
				setStats(newStats);
			}
	...
	const handleRevert = async (id: string) => {
		try {
			const res = await fetch(`/api/memes/${id}/revert`, {
				method: 'PATCH',
			});
			const result = await res.json();
			if (!res.ok) throw new Error(result.error);

			toast.success('Meme reverted to pending');
			fetchMemes();
		} catch (error) {
			...
			<div className='max-w-7xl mx-auto'>
				...
				{isLoading ? (
					...
					<div className='bg-white rounded-3xl border border-slate-200 p-12 text-center'>
						...
						<p className='text-slate-500'>
							There are no memes matching your current filter.
						</p>
					</div>
				) : (
				...
				{bulkScheduleModalOpen && (
					<div className='fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm'>
						<div className='bg-white rounded-3xl shadow-2xl max-w-md w-full p-6'>
							<h3 className='text-2xl font-bold text-slate-900 mb-4'>
								Schedule {selectedMemes.size} Meme
								{selectedMemes.size > 1 ? 's' : ''}
							</h3>
							<p className='text-slate-600 mb-4'>
...
function MemeCard({
	...
	return (
		<div
			className={`group bg-white rounded-3xl border-2 overflow-hidden transition-all hover:shadow-2xl hover:shadow-indigo-500/10 hover:-translate-y-1 ${
				isSelected
		...
		>
			...
			<div className='p-5'>
				...
				<div className='space-y-2'>
					...
					{meme.status === 'scheduled' && (
						<div className='bg-indigo-50 rounded-xl p-3 flex items-center gap-2'>
							...
							<span className='text-xs font-bold text-indigo-600'>
								{new Date(meme.scheduled_time!).toLocaleString([], {
									dateStyle: 'short',
									timeStyle: 'short',
								})}
							</span>
					...
					{/* Revert Button for non-pending non-published memes */}
					{(meme.status === 'approved' ||
						meme.status === 'rejected' ||
						meme.status === 'scheduled') && (
						<button
							onClick={() => onRevert(meme.id!)}
							className='w-full flex items-center justify-center gap-2 py-2.5 bg-slate-100 text-slate-600 text-xs font-black rounded-xl hover:bg-slate-200 transition-colors uppercase tracking-tight'
						>
				...
				<div className='fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4'>
					<div className='bg-white rounded-3xl w-full max-w-md p-8 shadow-2xl border border-slate-200'>
						...
						<p className='text-slate-500 mb-6 font-medium'>
							Please provide a reason why this meme is being rejected. This will
			...
			{isScheduleModalOpen && (
				<div className='fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4'>
					<div className='bg-white rounded-3xl w-full max-w-md p-8 shadow-2xl border border-slate-200'>
						...
						<p className='text-slate-500 mb-6 font-medium'>
							Select a date and time to publish this meme as a story.
						</p>
						<input
							type='datetime-local'
							value={scheduledDate}
