			...
			{COLUMNS.map((col) => (
				<div key={col.status} className="kanban-column flex flex-col gap-4 min-w-[280px] max-w-[320px]">
					...
					<div className="flex items-center gap-2 px-1">
						<div className="h-4 w-20 bg-[#2d3a54] rounded animate-pulse" />
						<div className="h-5 w-5 bg-[#2d3a54] rounded-full animate-pulse" />
					</div>
					{/* Card skeletons */}
					<div className="flex flex-col gap-3">
						{[1, 2].map((i) => (
							<div
								key={i}
								className="bg-[#1a2234] border border-[#2d3a54] rounded-xl p-3 animate-pulse"
							>
								<div className="aspect-[9/16] w-full rounded-lg bg-[#2d3a54] mb-3" />
								<div className="h-4 w-3/4 bg-[#2d3a54] rounded mb-2" />
								<div className="h-3 w-1/2 bg-[#2d3a54] rounded" />
							</div>
						))}
					</div>
				</div>
		...
		return (
			<div className="flex-1 flex items-center justify-center p-6">
				<div className="text-center">
					<div className="p-6 bg-[#1a2234] rounded-full mb-6 border border-[#2d3a54] inline-flex">
						<Loader2 className="h-12 w-12 text-[#92a4c9]" />
					</div>
