...
export function KanbanCard({ item, onPreview, onEdit, status }: KanbanCardProps) {
	...
	const cardStyles: Record<PublishingStatus, string> = {
		draft: 'bg-[#1a2234] border-[#2d3a54] hover:border-[#2b6cee]',
		scheduled: 'border-[#2b6cee]/40 bg-[#2b6cee]/5 hover:border-[#2b6cee]',
		processing: 'bg-[#1a2234] border-[#2d3a54]',
		published: 'bg-[#1a2234] border-[#2d3a54] hover:border-emerald-500/50',
	...
	return (
		...
		>
			...
			<div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
				<GripVertical className="h-5 w-5 text-[#92a4c9] cursor-grab" />
			</div>

			{/* Thumbnail - 9:16 aspect ratio */}
			<div
				className={cn(
					'w-full rounded-lg bg-cover bg-center mb-3 border border-[#2d3a54]/50 aspect-[9/16] relative overflow-hidden',
			...
			>
				{!imageError && item.mediaUrl ? (
					...
					<div className="absolute inset-0 flex items-center justify-center bg-[#232f48]">
						<span className="text-[#92a4c9] text-xs">No image</span>
					</div>
				)}
			...
			<div className="flex flex-col gap-2">
				{/* Title */}
				<h4 className="text-white text-sm font-bold leading-tight truncate">
				...
				{status === 'draft' && (
					<div className="flex items-center justify-between">
						<span className="text-[10px] text-[#92a4c9] flex items-center gap-1">
							<Pencil className="h-3 w-3" />
							Edited {formatRelativeTime(item.updatedAt)}
						</span>
						<div className="flex -space-x-1.5">
							<div className="h-5 w-5 rounded-full border border-[#1a2234] bg-blue-600 flex items-center justify-center text-[8px] font-bold text-white">
								{initials}
							</div>
						</div>
					</div>
				...
				{status === 'processing' && (
					<div className="flex flex-col gap-3">
						<div className="flex items-center justify-between">
							<h4 className="text-white text-sm font-bold truncate">{item.title || 'Processing...'}</h4>
						...
						<p className="text-[10px] text-[#92a4c9] italic">Processing media...</p>
					</div>
				)}
