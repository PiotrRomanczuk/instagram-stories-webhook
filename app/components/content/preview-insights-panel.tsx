...
import {
	Eye, Users, Loader2, BarChart3, RefreshCw, TrendingUp,
	ArrowRight, ArrowLeft, LogOut, MessageCircle,
} from 'lucide-react';
...
> = {
	impressions: { label: 'Impressions', icon: Eye, description: 'Total number of times the story was seen' },
	reach: { label: 'Reach', icon: Users, description: 'Number of unique accounts that saw the story' },
	replies: { label: 'Replies', icon: MessageCircle, description: 'Number of replies to the story' },
	taps_forward: { label: 'Taps Forward', icon: ArrowRight, description: 'Number of taps to see the next story' },
	taps_back: { label: 'Taps Back', icon: ArrowLeft, description: 'Number of taps to see the previous story' },
	exits: { label: 'Exits', icon: LogOut, description: 'Number of times someone exited the story' },
};
		...
		<section className="space-y-4">
			...
			<div className="grid grid-cols-2 gap-3">
				{insights.map((insight) => {
					...
					return (
						<div key={insight.name} className="p-4 bg-gradient-to-br from-indigo-50 to-purple-50 rounded-2xl border border-indigo-100/50 group hover:shadow-md transition-all" title={config?.description || insight.description}>
