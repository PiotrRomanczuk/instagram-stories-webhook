...
interface MediaPanelProps {
	...
	onToggleStoryFrame: (value: boolean) => void;
}

/**
 * Left side media panel with Original / Story View toggle
 */
export function MediaPanel({ item, showStoryFrame, onToggleStoryFrame }: MediaPanelProps) {
	return (
		<div className='h-[280px] flex-shrink-0 md:h-auto md:flex-1 bg-gray-950 relative flex items-center justify-center overflow-hidden'>
			{showStoryFrame ? (
				<div className='h-full w-full aspect-[9/16] max-h-full flex items-center justify-center relative overflow-hidden'>
					...
					{item.mediaType === 'VIDEO' ? (
						<UniversalVideoPlayer url={item.mediaUrl} thumbnailUrl={item.thumbnailUrl} className='relative z-10 max-h-full max-w-full' contain playing={true} />
					) : (
				...
				<div className='relative w-full h-full flex items-center justify-center bg-gray-950'>
					{item.mediaType === 'VIDEO' ? (
						<UniversalVideoPlayer url={item.mediaUrl} thumbnailUrl={item.thumbnailUrl} className='w-full h-full' contain playing={true} />
					) : (
			...
			<div className='absolute bottom-8 left-1/2 -translate-x-1/2 z-40 bg-white/10 backdrop-blur-xl border border-white/10 rounded-2xl p-1 shadow-2xl flex'>
				<button
					onClick={() => onToggleStoryFrame(false)}
				...
				<button
					onClick={() => onToggleStoryFrame(true)}
					className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${showStoryFrame ? 'bg-white text-black shadow-lg' : 'text-white/60 hover:text-white'}`}
				>
