...
import { Clock, Loader, Play, PlayCircle, Trash2 } from 'lucide-react';
...
export function PendingPostsPanel() {
	...
	const handleDeleteSingle = async (postId: string) => {
		...
		try {
			const res = await fetch('/api/developer/cron-debug/delete-posts', {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify({
					postIds: [postId],
	...
	const handleDeleteAll = async () => {
		...
		if (
			!confirm(
				`Are you sure you want to delete all ${data.count} overdue posts?`,
			)
		)
			return;
				...
				<div>
					...
					{data.count > 0 && (
						<div className='flex flex-col sm:flex-row gap-3 mb-4 p-4 bg-slate-100 rounded-lg border border-slate-200'>
							...
							>
								...
								Process All ({data.count})
							</button>

							<button
								onClick={handleDeleteAll}
								disabled={bulkProcessing}
								className='flex-1 px-4 py-3 bg-rose-600 text-white rounded-lg font-bold hover:bg-rose-700 disabled:opacity-50 transition flex items-center justify-center gap-2'
							>
								{bulkProcessing ? (
									<Loader className='w-5 h-5 animate-spin' />
								) : (
					...
					{data.count > 0 && (
						<p className='text-xs text-slate-500 mb-4 italic'>
							⚠️ Force-processing bypasses duplicate detection and processes
							immediately
						</p>
					)}
										...
										<div className='flex flex-wrap items-center gap-2'>
											...
											<div className='flex gap-2'>
												...
												<button
													...
													className='px-3 py-1.5 bg-rose-600 text-white rounded-lg font-bold text-xs hover:bg-rose-700 disabled:opacity-50 transition flex items-center gap-1.5'
												>
													{deletingIds.has(post.id) ? (
														<Loader className='w-3.5 h-3.5 animate-spin' />
													) : (
