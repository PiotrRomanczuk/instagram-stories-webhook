					...
					{options.map((option) => (
						<button
							...
							className={cn(
								...
								option.value === value ? 'text-[var(--sf-primary)] font-medium' : 'text-slate-700'
							)}
						>
							{option.label}
						</button>
...
export function SfToolbarToggle({ label, checked, onChange, className }: SfToolbarToggleProps) {
	return (
		<label className={cn('flex cursor-pointer items-center gap-2 text-sm font-medium text-slate-700', className)}>
			<button
				type="button"
				role="switch"
				...
				className={cn(
					...
					checked ? 'bg-[var(--sf-primary)]' : 'bg-slate-300'
				)}
			>
				<span className={cn(
		...
		<div className={cn('relative', className)}>
			...
			{value && (
				<button
					type="button"
					onClick={() => onChange('')}
					className="absolute right-3 top-1/2 -translate-y-1/2 text-[var(--sf-text-secondary)] hover:text-slate-700"
				>
					<X className="h-4 w-4" />
				</button>
			)}
