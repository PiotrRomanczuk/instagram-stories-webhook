...
describe('Memes API Routes', () => {
    ...
    describe('/api/memes (List/Submit)', () => {
        const userSession = { user: { id: 'u1', email: 'u1@test.com', role: 'user' } };
        const adminSession = { user: { id: 'a1', email: 'a1@test.com', role: 'admin' } };
        ...
        describe('GET', () => {
            ...
            it('should filter by own userId for regular users', async () => {
                ...
                expect(getMemeSubmissions).toHaveBeenCalledWith(expect.objectContaining({ userId: 'u1' }));
            });
            ...
            it('should not filter by userId for admins', async () => {
                ...
                expect(getMemeSubmissions).toHaveBeenCalledWith(expect.not.objectContaining({ userId: 'a1' }));
            });
        });
        ...
        describe('POST', () => {
            ...
            it('should return 400 for invalid data', async () => {
                (getServerSession as Mock).mockResolvedValue(userSession);
                const req = createRequest('POST', '/api/memes', { media_url: 'invalid' });
                const res = await POST_SUBMIT(req);
    ...
    describe('/api/memes/[id] (Single/Review/Delete)', () => {
        ...
        describe('GET', () => {
            it('should return meme if owner', async () => {
                ...
                const res = await GET_SINGLE(createRequest('GET', '/api/memes/m1'), { params: Promise.resolve({ id: 'm1' }) });
                expect(res.status).toBe(200);
            });
        ...
        describe('PATCH (Review)', () => {
            it('should allow admin to review', async () => {
                (getServerSession as Mock).mockResolvedValue(adminSession);
                (reviewMemeSubmission as Mock).mockResolvedValue({ ...mockMeme, status: 'approved' });

                const body = { action: 'approve' };
                const res = await PATCH_REVIEW(createRequest('PATCH', '/api/memes/m1', body), { params: Promise.resolve({ id: 'm1' }) });
                
                expect(res.status).toBe(200);
                expect(reviewMemeSubmission).toHaveBeenCalledWith('m1', 'a1', 'approve', undefined);
            });

            it('should block non-admin from review', async () => {
                (getServerSession as Mock).mockResolvedValue(userSession);
                const res = await PATCH_REVIEW(createRequest('PATCH', '/api/memes/m1', { action: 'approve' }), { params: Promise.resolve({ id: 'm1' }) });
                expect(res.status).toBe(403);
            });
        });
        ...
        describe('DELETE', () => {
            ...
            it('should block owner from deleting approved meme', async () => {
                (getServerSession as Mock).mockResolvedValue(userSession);
                (getMemeSubmission as Mock).mockResolvedValue({ ...mockMeme, status: 'approved' });
