		...
		if (appId && appSecret) {
			try {
				...
				if (debugTokenRes.data.data.granular_scopes) {
					const pageScopes = (
						debugTokenRes.data.data.granular_scopes as GranularScope[]
					).filter((s) => s.scope && s.scope.includes('pages'));
					pageIds = Array.from(
						new Set(
							pageScopes.flatMap((s) => (s.target_ids || []) as string[]),
						),
					) as string[];
		...
		for (const page of pages) {
			...
			try {
				...
				const errorMessage = axios.isAxiosError(err)
					...
					: err;
				console.error(
					`Error checking Instagram for page ${page.name}:`,
					errorMessage,
				);
			}
		}
...
export async function getInstagramUsername(
	...
	try {
		...
		// console.error('Error fetching IG Username:', error);
		return null;
	}
}
