...
export async function getScheduledPosts(
	...
	return (data || []).map((post) =>
		mapScheduledPostRow(post as ScheduledPostRow),
	);
}
...
export async function updateScheduledPost(
	...
	if (updates.idempotencyKey)
		dbUpdates.idempotency_key = updates.idempotencyKey;
	if (updates.memeId) dbUpdates.meme_id = updates.memeId;
	if (updates.retryCount !== undefined)
		dbUpdates.retry_count = updates.retryCount;
...
export async function deleteScheduledPosts(ids: string[]): Promise<number> {
	...
	if (error) {
		console.error('Supabase deleteScheduledPosts Error:', error);
		return 0;
	}
...
export async function acquireProcessingLock(postId: string): Promise<boolean> {
	...
	const processingStartedAt = post.processing_started_at
		? new Date(post.processing_started_at)
		: null;
	...
	if (!canAcquire) {
		console.log(
			`[Lock] Cannot acquire lock for ${postId}: status=${status}, processing_started_at=${processingStartedAt?.toISOString()}, threshold=${timeoutThreshold.toISOString()}`,
		);
		return false;
	}
